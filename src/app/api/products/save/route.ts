  ...
  try {
    ...
    if (error) {
      ...
      if (isRpcOrSchemaError) {
        console.warn('[Product Save Fallback] RPC save_product_transaction indisponível no Supabase. Executando persistência direta segura via servidor:', errMessage);

        const prodId = product.id || crypto.randomUUID();
        const basePayload: any = {
          name: rawName,
          category: product.category,
          price_balcao: priceBalcao,
          price_ifood: priceIfood,
          is_active: product.isActive !== false,
          status: product.status || 'validado',
        };
        if (product.subcategory) basePayload.subcategory = product.subcategory.trim();

        // 1. Atualizar ou Inserir Produto no banco
        let pErr: any = null;
        if (product.id) {
          const { error: updErr } = await db.from('products').update(basePayload).eq('id', prodId);
          if (updErr) {
            delete basePayload.subcategory;
            delete basePayload.status;
            const { error: updErr2 } = await db.from('products').update(basePayload).eq('id', prodId);
            if (updErr2) pErr = updErr2;
          }
        } else {
          basePayload.id = prodId;
          const { error: insErr } = await db.from('products').insert(basePayload);
          if (insErr) {
            delete basePayload.subcategory;
            delete basePayload.status;
            const { error: insErr2 } = await db.from('products').insert(basePayload);
            if (insErr2) pErr = insErr2;
          }
        }

        if (pErr) {
          console.error('[Product Save Fallback Error]:', pErr);
          throw new AccessError(500, `Falha ao gravar produto no banco: ${pErr.message}`);
        }

        // 2. Atualizar Receita (Limpa e recria no servidor de forma atômica)
        await db.from('recipes').delete().eq('product_id', prodId);

        if (sanitizedRecipe.length > 0) {
          const recipeRowsWithCols = sanitizedRecipe.map(r => ({
            product_id: prodId,
          ...
          const { error: rInsErr } = await db.from('recipes').insert(recipeRowsWithCols);
          if (rInsErr) {
            console.warn('[Product Save Fallback] Tentando gravar receita sem colunas novas de produção:', rInsErr.message);
            const minimalRecipeRows = sanitizedRecipe.map(r => ({
              product_id: prodId,
              ingredient_id: r.ingredientId,
              quantity: r.quantity,
            }));
            const { error: rInsErr2 } = await db.from('recipes').insert(minimalRecipeRows);
            if (rInsErr2) {
              console.error('[Product Save Fallback Recipe Error]:', rInsErr2);
              throw new AccessError(500, `Falha ao gravar receita no banco: ${rInsErr2.message}`);
            }
          }
        }
        ...
        try {
          await db.from('audit_logs').insert({
            action: 'GRAVACAO_PRODUTO_RECEITA',
            details: `Produto "${rawName}" gravado via fallback de servidor.`,
